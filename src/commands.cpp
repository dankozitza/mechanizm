...
// better_default_help
//
// When this function is set to the "help" command output is not written to
// stdout. instead lines of output overwritten into argv. no newlines are used.
// Argv is cleared then each line of output is appended to the vector. If argv
// is empty it will write the help message for the entire commands object,
// `cmds_help`, then a list of the available commands.
//
// Ex:
//
//    commands cmds;
//    cmds.handle("help", better_default_help,
...
//
// If the argv vector has arguments in it the usage, synopsis, and description
// for those commands are overwritten into argv. multiple command names
// can be listed.
//
void commands::better_default_help(vector<string>& argv) {
   is_resolved = false;
   vector<string> vout;
   if (argv.size() == 0) {

      vout.push_back(cmds["help"].synopsis);
      vout.push_back("");
      ...
      for (int i = 0; i < argv.size(); i++) {
         if (cmds.find(argv[i]) != cmds.end()) {
            ...
            if (argv[i] == "help") {
               argv.clear();
               better_default_help(argv);
            }
            else {
               vout.push_back("Unknown command `");
               vout[vout.size()-1] += argv[i];
               vout[vout.size()-1] += "`. Try 'help'.\n";
            }
         }
      }
   }
   tools::newlines_to_indices(vout);
   argv = vout;
}
...
void commands::run(string cmd, vector<string>& arguments) {
   ...
   if (cmd == "help" && it == cmds.end())
      ...
      cout << "commands::run: Unknown command `" << cmd << "\n";
   }
}
